...
import { resolveFnTarget, executeFnFrame } from './fn-frame'
...
import { runHttpWorkflow as runHttpWorkflowCore } from '../../lib/http-workflow-graph/run-http-workflow.mjs'
...
export interface ExecutionContext {
  ...
  /** R11 fn 调用帧深度（根图 0，每层函数 +1；超 MAX_FN_DEPTH 拒执行防递归） */
  fnDepth?: number
}

export interface RunContext {  // 多轮 Chat 执行上下文 — 贯穿 WorkingMemory / PromptInput / HttpWorkflow
...
export async function executeNode(
  ...
  // R11 函数节点：fn_ref / subgraph / def.fn_ref 命中即走调用帧（优先于注册执行器）
  const fnTarget = resolveFnTarget(node)
  const executor: ExecutorFn | undefined = fnTarget
    ? (n, i, c) => executeFnFrame(n, i, c, fnTarget)
    : executorRegistry.get(node.class_type)
...
registerExecutor('TextTransform', async (node, inputs) => {  // 文本处理：原子化元件：输入文本 → 变换（截取/替换/格式化）→ 输出文本。 | 入:text 出:result
  ...
  switch (op) {
    ...
    case 'split':  // 按行或 pattern 正则拆分，去空行
    case 'split_lines': {
      const pattern = String(node.params.pattern ?? '').trim()
      const delim = pattern ? new RegExp(pattern) : /\r?\n/
      const lines = text.split(delim).map(s => s.trim()).filter(Boolean)
      result = lines
...
registerExecutor('Delay', async (node, inputs) => {  // 延时：原子化元件：输入信号 → 等待 N 秒 → 透传输出。 | 入:trigger 出:pass
  const hasSeconds = node.params.seconds !== undefined && node.params.seconds !== null
  const seconds = hasSeconds
    ? Number(node.params.seconds)
    : node.params.delay_ms != null
      ? Number(node.params.delay_ms) / 1000
      : 1
  const ms = Math.min(seconds * 1000, 5000)
  await new Promise(r => setTimeout(r, ms))
  return { outputs: { pass: inputs.trigger ?? true, done: true }, duration_ms: ms }
})
...
async function runHttpWorkflow(
  ...
  return runHttpWorkflowCore({
    params,
    inputs,
    ctx: { runContext: ctx.runContext as Record<string, unknown> | undefined },
  })
}
