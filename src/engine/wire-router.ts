...
function safeStubX(ax: number, ay: number, dx: number, obs: AABB[]): number {
...
function mergeSharedOutputPaths(
  ...
  for (const [, group] of groups) {
    ...
    let junctionX = Math.max(srcStubX + TRUNK_MIN, Math.min(midX, minDestStubX - STUB * 0.5))
    ...
    for (const dest of dests) {
      const checkObs = obstacles.filter((_, i) =>
        i !== srcObsIdx && i !== nodeObstacleIdx.get(dest.toNodeId),
      )
      ...
      const hBranch = hSegBlocked(dest.to.y, junctionX, dest.to.x, checkObs)

      if (vBlocked || hTrunk || hBranch) continue
      ...
      paths.set(dest.linkId, simplify(path))
    }
  }
}
...
export function routeSingleDrag(
  ...
  if (startId === undefined || endId === undefined) {
    return buildFallbackPath(from, cursorPos)
  }
...
export function routeAllLinks(
  ...
  const maxBottom = obstacles.reduce((m, o) => Math.max(m, o.y + o.h), 0)
  const bottomLaneY = maxBottom + BUFFER * 3
  ...
  for (const fl of forwardLinks) {
    ...
    const endId = grid.idx.get(`${stubTo.x},${stubTo.y}`)
    if (startId === undefined || endId === undefined) {
      result.set(link.id, buildFallbackPath(from, to))
      continue
    }
    const pathIds = astar(grid, startId, endId)
    if (pathIds.length === 0) {
  ...
  for (const fl of backwardLinks) {
    ...
    if (legDown.length > 0) {
      ...
      pts.push(stubFrom, { x: stubFrom.x, y: bottomLaneY })
    }
    if (legAcross.length > 0) {
      for (let i = 1; i < legAcross.length; i++) pts.push({ x: grid.pts[legAcross[i]].x, y: grid.pts[legAcross[i]].y })
    } else {
      pts.push({ x: stubTo.x, y: bottomLaneY })
    }
    if (legUp.length > 0) {
