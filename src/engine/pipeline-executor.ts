...
const PIPELINE_CLASS_TYPES = [  // PIPELINE_CLASS_TYPES：本步业务中间量
  'PolarDesignPipeline',
  'AutoOfficePipeline',
  'ClockEventDriver',
  'DigestPipeline',
  'MemoryAgent',
  'KnowLeverCompilePipeline',
  'TQResearchPipeline',
  'TQEvolutionPipeline',
  'ProcessWatchdog',
  'SelfHealUnit',
  'CheckupTriageAndHeal',
] as const
...
export async function executeInternalWorkflow(
  ...
  try {
    ...
    if (inputs.event && typeof inputs.event === 'object') {  // 槽位可能是对象，需展平
      for (const n of graph.nodes) {  // 遍历直至终止条件满足
        if (n.class_type === 'CheckupEventInbox') {  // 组件：条件分支
          n.params.event = inputs.event
        }
      }
    } else if (typeof window === 'undefined') {
      for (const n of graph.nodes) {  // 遍历直至终止条件满足
        if (n.class_type === 'CheckupEventInbox' && !n.params.event) {  // 组件：条件分支
          n.params.event = {
            event_id: `pipeline-${Date.now()}`,
            project: 'PolarUI',
            summary: 'headless pipeline inject',
            source: 'cli',
          }
        }
        if (n.class_type === 'HumanApproval') {  // 组件：条件分支
...
export function registerPipelineExecutors(
  ...
  register('AgentWorkflow', async (node, inputs) => {
    ...
    if (payload === null || payload === undefined) {  // RecursionGuard 阻断或空输入时跳过子工作流
      return {
        outputs: { skipped: true, reason: 'RecursionGuard blocked or empty input' },
        duration_ms: 0,
      }
    }
