...
describe('taoci-outreach graph (structure)', () => {
  it('lg.json 无 ShellExec，含 Switch + TaociSessionLoad', () => {
    ...
    assert.ok(types.includes('FeishuIM'));
  });
  ...
  it('graph engine 执行 S0 路径（mock LLM）', async () => {
    ...
    // LG 单路径：S0 新会话不应跑 S1 SubAgent
    assert.ok(!result.node_traces.includes('TaociSubAgent'), 'LG must run one branch only');
    assert.equal(result.node_traces.filter((t) => t === 'TaociSessionSave').length, 1);
  });
});
