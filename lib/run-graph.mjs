...
export async function runWorkflowGraph(opts) {
  ...
  const { executeGraph, parseWorkflow } = engine;
  if (opts.workflowId === 'native-web-qa') {
    const { registerNativeWebQaFixture } = await import('./native-web-qa-fixture.mjs');
    registerNativeWebQaFixture(engine.registerExecutor);
  }
  const path = opts.workflowPath ?? resolveWorkflowPath(opts.workflowId ?? '');
  const raw = readFileSync(path, 'utf8');
  const graph = parseWorkflow(raw);

  const { conversationId, message, files = [], userId, memory, history = [], command = {} } = opts.inputs;
