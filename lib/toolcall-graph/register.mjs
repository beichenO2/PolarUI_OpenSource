 ...
 * 执行走 _lg_edges 条件路由到明面工具节点（outputs.branch = tool name）。
 */
import { join, dirname } from 'node:path';
import { fileURLToPath, pathToFileURL } from 'node:url';
import { ToolList, scanSkillsDir, skillSearch } from '../toolcall-composite.mjs';
import { parseToolList } from '../toolcall-gui/tool-list.mjs';
import { resolveMainBundle } from '../bundle-assets.mjs';
...
export function registerToolcallComposite(registerExecutor) {
  ...
  registerExecutor('ToolCall', async (node, inputs, ctx) => {
    ...
    const fromParams = parseToolList(node.params?.tool_list);
    for (const def of fromParams) {
      toolList.addTool({ name: def.name, desc: def.desc, parameters: def.parameters });
    }
    ...
    if (!bundle) {
      return {
        ...
        duration_ms: Date.now() - start,
        error: 'PolarUI bundle not found',
      };
    }
