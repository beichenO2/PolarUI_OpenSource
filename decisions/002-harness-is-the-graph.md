...
1. **Harness = workflow 图（`.json`）** — 状态机、LLM、SubAgent、WorkingMemory 等**必须在图上可见**。图含 `_entry`/`_lg_edges` 时引擎自动步进单路径（ADR-010）；`.lg.json` 仅为兼容读取。
...
| 现在（错） | 目标（对） |
|------------|------------|
| `taoci-route.ts` → `spawnSync('node', harness/index.mjs)` | `taoci-route.ts` → PolarUI `executeGraph(taoci-outreach.json, inputs)` |
| 逻辑在 `harness/state-machine.mjs` | 逻辑在 workflow `.json` 节点与边 |

## 后果

- taoci-outreach 重写：`WorkingMemory → Switch(step) → LLM/SubAgent → FeishuIM → Output`
- `harness/` 文件夹逻辑上浮到图后删除或仅留测试 mock
- `WORKFLOW.spec.md` 架构图以 `{id}.json` 为准
